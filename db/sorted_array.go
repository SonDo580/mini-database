...
func (arr *SortedArray) Iter() (SortedKVIter, error) {
	return &SortedArrayIter{keys: arr.keys, vals: arr.vals, pos: 0}, nil
}
...
func (arr *SortedArray) Get(key []byte) (val []byte, ok bool, err error) {
	if idx, exists := slices.BinarySearchFunc(arr.keys, key, bytes.Compare); exists {
...
func (arr *SortedArray) Set(key []byte, val []byte) (updated bool, err error) {
	idx, exists := slices.BinarySearchFunc(arr.keys, key, bytes.Compare)
	if exists {
		arr.vals[idx] = val
	} else {
